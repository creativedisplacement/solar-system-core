...
namespace SolarSystemCore.Tests.Services
{
    ...
    public class MoonServiceTests
    {
        ...
        {
            var result = await _service.GetMoonsByPlanetId(_moons.FirstOrDefault().PlanetId);
            Assert.IsNotNull(result);
        ...
        public async Task Service_GetAllMoonsByPlanetId_ReturnsUnexpectedMoon()
        {
            ...
            var result = await _service.GetMoonsByPlanetId(_moons.Skip(1).Take(1).FirstOrDefault().PlanetId);
            Assert.IsNotNull(result);
            Assert.AreEqual(1, result.Count());
